...
studentas::studentas(const studentas& s) : //kopijavimo konstruktorius
    vardas_{ s.vardas_ }, pavarde_{ s.pavarde_ }, egz_{ s.egz_ }, galutinis_vidurkis{ s.galutinis_vidurkis },  paz_{ s.paz_ }
{
    for (int i = 0; i < s.GetNdDydi(); i++)
    {
        paz_.push_back(s.GetNdElementa(i));
    }
}
studentas& studentas::operator=(const studentas& s) //copy priskyrimas
{
    ...
    for (int i = 0; i < s.GetNdDydi(); i++)
    {
        nd_.push_back(s.GetNdElementa(i));
    }
    s.~studentas();
    return *this;
}
